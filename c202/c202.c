...
void Stack_Init( Stack *stack ) {
	...
	// pokud je stack NULL, zavola se funkce Stack_Error(SERR_INIT)
	if (!stack) {
		Stack_Error(SERR_INIT);
		return;
	}

	// nastavi se topIndex na -1 coz je hodnota odpovidajici prazdnemu zasobniku
	stack -> topIndex = -1;
...
bool Stack_IsEmpty( const Stack *stack ) {
	...
	// pokud se topIndex nerovna -1 tedy neznaci prazdny zasobnik, vrati se false, jinak true
...
bool Stack_IsFull( const Stack *stack ) {
	...
	// maximalni pocte prvku je STACK_SIZE - 1
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	// jestlize je zasobnik prazdny, zavola se Stack_Error(SERR_TOP)
	if (Stack_IsEmpty(stack)) {
		Stack_Error(SERR_TOP);
		return;
	}
	
	// nastavi se jako hodnota znak z vrcholu zasobniku
...
void Stack_Pop( Stack *stack ) {
	...
	// pokud neni zasobnik prazdny, tak ma smysl prvek odstranit, tedy zmensime topIndex
	if (!Stack_IsEmpty(stack)) {
		stack->topIndex--;
	}
...
void Stack_Push( Stack *stack, char data ) {
	...
	// pokud je zasobnik plny, volame funkci Stack_Error(SERR_PUSH)
	if(Stack_IsFull(stack)) {
		Stack_Error(SERR_PUSH);
		return;
	}

	// zvysime topIndex a na jeho pozici vlozime dany znak
	stack->topIndex++;
